...
A condition `RotationStatus` gives feedback to the current configuration of the rotation paramters. It can have the values `Disabled`, `Enabled` and `EnabledWithWarning`. `Enabled` means that there is a rotation planned. `Disabled` means that no rotation is planned. `EnabledWithWarning` means that rotation is enabled with a valid configuration, but it may be senseless or unwanted: 
`ttl` and `frequency` can be chosen in a way that makes it possible to have more than one `retiredKey` item at a time. Potential senseless configurations like `{ttl: 24h, frequency: 1m}` would result in up to 360 instances existing at the same time. If such rotation behaviour should be wanted anyway, the warning can be ignored.
...
1) There are two reasons why a resource might be retired.
(1) As part of the regular rotation intervals: If the lifetime of the current instance is longer than the duration of `.spec.rotation.frequency`, and
(2) if force rotation is ordered: If the resource is annotated with at specific annotation (for `ServiceBinding` this is `servicebinding.account.btp.crossplane.io/force-rotation=true`).
If either of those two conditions is fulfilled, the rotation starts. For this, the current instance is added to the `.status.retireKeys` array with it's respective `.createdAt`, `.id` and `.name` attributes and `.retiredKeys` (=`now()`) and `.deletionDate` (=`.createdDate+.spec.rotation.ttl`). Afterwards, return with `ResourceExists: false`. This will immediately trigger a creation of the new instance in `Create()`.
