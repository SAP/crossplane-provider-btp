...
// applyFailure builds upjet v1's ApplyFailed for a terraform diagnostic, which
// NewApplyFailed expects as a terraform-log JSON line.
func applyFailure(message string) error {
	return tferrors.NewApplyFailed([]byte(fmt.Sprintf(`{"@level":"error","@message":%q}`, message)))
}

// createFailure and updateFailure are what the async callback records after
// tagging upjet v1's ApplyFailed with the finished operation.
func createFailure(message string) error {
	return tferrors.NewAsyncCreateFailed(applyFailure(message))
}

func updateFailure(message string) error {
	return tferrors.NewAsyncUpdateFailed(applyFailure(message))
}

// launchRefusal is upjet v1's answer to Create/Update while the previous apply
// is still running.
const launchRefusal = "cannot start async apply: apply operation that started at X is still running"
...
func TestObserve(t *testing.T) {
	...
	rejectionMessage := updateFailure("Cannot change AppId with update").Error()
	conflictWordingMessage := updateFailure("API Error Updating Resource Service Instance (Subaccount): Conflict").Error()
	...
	cases := map[string]struct {
		...
		"AsyncUpdateFailure_HoldsUnhealthy": {
			...
			args: args{
				mg: expectedServiceInstance(
					withObservationData("some-id", ""),
					withAsyncFailureCondition(ujresource.ReasonAsyncUpdateFailure, rejectionMessage),
				),
			},
			want: want{
				err: nil,
				...
				cr:               expectedServiceInstance(withObservationData("some-id", "")),
				wantReadyReason:  reasonAsyncOperationFailed,
				wantReadyStatus:  corev1.ConditionFalse,
				wantReadyMessage: rejectionMessage,
			},
		},
		...
		"AsyncUpdateFailure_ConflictWording": {
			...
			args: args{
				mg: expectedServiceInstance(
					withObservationData("some-id", ""),
					withAsyncFailureCondition(ujresource.ReasonAsyncUpdateFailure, conflictWordingMessage),
				),
			},
			want: want{
				err: nil,
		...
		// reconciler would apply, so holding on it must stop once Generation
		// moves past the condition's ObservedGeneration.
		"AsyncUpdateFailure_SpecChanged": {
			...
			args: args{
				mg: func() *v1alpha1.ServiceInstance {
					cr := expectedServiceInstance(withObservationData("some-id", ""))
					cr.Generation = 1
					withAsyncFailureCondition(ujresource.ReasonAsyncUpdateFailure, rejectionMessage)(cr)
					cr.Generation = 2
					return cr
				}(),
			},
		...
		// A rejected create must not be held; NotExisting is reported as usual
		// so Create() runs again.
		"AsyncCreateFailure_StillCreates": {
			...
			args: args{
				mg: expectedServiceInstance(
					withAsyncFailureCondition(ujresource.ReasonAsyncCreateFailure, createFailure("quota exceeded").Error()),
				),
			},
			want: want{
				err: nil,
		...
		// Adopting after a failed create records the instance's ID while that
		// failure is still current; it must not be held as a rejected update.
		"AsyncCreateFailure_ObservedInstance_NotHeld": {
			reason: "should not hold a create failure unhealthy once the instance has been observed",
			fields: fields{
				client: &TfProxyMock{status: tfclient.UpToDate, details: map[string][]byte{}},
			},
			args: args{
				mg: expectedServiceInstance(
					withExternalName("6c5a4e0e-7d1b-4f3a-9a2e-8b0d5f1c3a77"),
					withObservationData("6c5a4e0e-7d1b-4f3a-9a2e-8b0d5f1c3a77", ""),
					withAsyncFailureCondition(ujresource.ReasonAsyncCreateFailure, createFailure("Conflict: resource already exists").Error()),
				),
			},
			want: want{
				err: nil,
...
func TestCreate(t *testing.T) {
	...
	cases := map[string]struct {
		...
		// The marker is written before the launch, so it must survive a
		// launch that upjet refuses.
		"LaunchRefused_StillRecordsTheStart": {
			reason: "should record the operation start when upjet refuses the launch",
			fields: fields{
				client: &TfProxyMock{err: errors.New(launchRefusal)},
			},
			args: args{
				mg: &v1alpha1.ServiceInstance{},
			},
			want: want{
				err: errors.New(errCreateInstance + ": " + launchRefusal),
				cr: expectedServiceInstance(
					withAsyncOperationMarker(0),
					withConditions(xpv1.Creating()),
				),
...
func TestUpdate(t *testing.T) {
	...
	cases := map[string]struct {
		...
		"RejectedUpdate_ReportsTheRejection": {
			reason: "should report the recorded rejection so the resource cannot go back to Synced=True while the retry runs",
			fields: fields{
				client: &TfProxyMock{},
			},
			args: args{
			...
			want: want{
				err: errors.New(rejectionMessage),
				cr: expectedServiceInstance(
					withObservationData("some-id", ""),
					withAsyncOperationMarker(0),
				),
		...
		"RejectedUpdate_InFlight": {
			reason: "should report the rejection alongside upjet's refusal to start a second attempt",
			fields: fields{
				client: &TfProxyMock{err: errors.New(launchRefusal)},
			},
			args: args{
			...
			want: want{
				err: errors.New(rejectionMessage + ": " + launchRefusal),
				cr: expectedServiceInstance(
					withObservationData("some-id", ""),
					withAsyncOperationMarker(0),
				),
...
func TestOperationGenerationSurvivesCreateReconcile(t *testing.T) {
	...
	r := managed.NewReconciler(
		...
		managed.WithExternalConnecter(managed.ExternalConnectorFn(
	...
	// caused.
	failure := createFailure("Conflict: resource already exists")
	if err := saveCallback(context.Background(), kube, siClient.TfNamePrefix+key.Name,
...
func TestFastCallbackKeepsTheCurrentGeneration(t *testing.T) {
	rejection := updateFailure("Cannot change AppId with update")
	...
	cr := expectedServiceInstance(withAsyncOperationMarker(1), withObservationData("some-id", ""))
	cr.Name = "test-instance"
	cr.Generation = 2
	...
	tf := &callbackDuringUpdate{
		...
		run: func() {
			if err := saveCallback(context.Background(), kube, siClient.TfNamePrefix+key.Name,
				ujresource.LastAsyncOperationCondition(rejection), ujresource.AsyncOperationFinishedCondition()); err != nil {
	...
	if synced := settled.GetCondition(xpv1.TypeSynced); synced.Status != corev1.ConditionFalse {
		t.Errorf("expected Synced=False, got %q/%q (%s)", synced.Status, synced.Reason, synced.Message)
	}
}
...
func TestReconcileRejectedUpdate(t *testing.T) {
	rejection := updateFailure("Cannot change AppId with update").Error()
	...
	cases := map[string]struct {
		...
		"RetryInFlight": {
			...
			client: &TfProxyMock{
				status:    tfclient.UpToDate,
				updateErr: errors.New(launchRefusal),
			},
...
func TestSaveCallback(t *testing.T) {
	...
	failure := ujresource.LastAsyncOperationCondition(updateFailure("Cannot change AppId with update"))
	...
	captureWrites := func(writes *[][]xpv1.Condition) test.MockSubResourceUpdateFn {
			...
			build: func() (args, func(t *testing.T)) {
				...
				return args{
						kube: &test.MockClient{
							...
							MockStatusUpdate: captureWrites(&writes),
						},
