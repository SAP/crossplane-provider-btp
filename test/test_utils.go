...
// PatchProviderConfigDestinationCredentials updates the default ProviderConfig
// to set (or clear) destinationCredentials pointing at the given secret.
// Pass empty secretName to clear the field.
func PatchProviderConfigDestinationCredentials(ctx context.Context, cfg *envconf.Config, secretName, secretNamespace string) error {
	r, err := resources.GetResourcesWithRESTConfig(cfg)
	if err != nil {
		return fmt.Errorf("get resources client: %w", err)
	}
	_ = metaApi.AddToScheme(r.GetScheme())

	pc := &apiV1Alpha1.ProviderConfig{}
	if err := r.Get(ctx, "default", "", pc); err != nil {
		return fmt.Errorf("get ProviderConfig: %w", err)
	}

	updated := pc.DeepCopy()
	if secretName == "" {
		updated.Spec.DestinationServiceSecret = nil
	} else {
		updated.Spec.DestinationServiceSecret = &apiV1Alpha1.ProviderCredentials{
	...
	if err := r.Update(ctx, updated); err != nil {
		return fmt.Errorf("update ProviderConfig: %w", err)
	}
	return nil
}
