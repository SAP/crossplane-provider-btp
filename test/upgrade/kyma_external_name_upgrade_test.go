...
// Test_KymaEnvironment_External_Name verifies that the KymaEnvironment
// external-name is correctly preserved and/or migrated during provider upgrades.
//
// The KymaEnvironment controller has backwards compatibility logic that migrates
// external-names from old formats (metadata.name or forProvider.name in older versions) to GUID format.
...
func Test_KymaEnvironment_External_Name(t *testing.T) {
	const kymaEnvironmentName = "upgrade-test-kyma-extn-env"

	upgradeTest := NewCustomUpgradeTest("kyma-environment-external-name-test").
		FromVersion(kymaFromCustomTag).
		ToVersion(kymaToCustomTag).
		WithResourceDirectories(kymaCustomResourceDirectories).
		WithCustomPreUpgradeAssessment(
			"verify external name before upgrade",
			func(ctx context.Context, t *testing.T, cfg *envconf.Config) context.Context {
				kymaEnv := &environmentv1alpha1.KymaEnvironment{}
				r := cfg.Client().Resources()

				err := r.Get(ctx, kymaEnvironmentName, cfg.Namespace(), kymaEnv)
				if err != nil {
					t.Fatalf("Failed to get KymaEnvironment resource: %v", err)
				}

				// Get the external name annotation
				annotations := kymaEnv.GetAnnotations()
				...
				klog.V(4).Infof("Pre-upgrade KymaEnvironment external name: %s", externalName)

				// Store the external name in context for post-upgrade verification
				// Note: In older versions, the external-name may be in metadata.name or forProvider.name format.
				// After upgrade, it should be migrated to GUID format.
				return context.WithValue(ctx, "preUpgradeKymaExternalName", externalName)
			},
		).
		WithCustomPostUpgradeAssessment(
			...
			func(ctx context.Context, t *testing.T, cfg *envconf.Config) context.Context {
				...
				klog.V(4).Infof("Post-upgrade KymaEnvironment external name: %s", externalName)

				// After upgrade, the external-name should be in GUID format
				// The controller migrates from name formats to GUID
				...
				preUpgradeExternalName, ok := ctx.Value("preUpgradeKymaExternalName").(string)
				if !ok {
				...
				// If it was in name format, it should have been migrated to GUID format
				if internal.IsValidUUID(preUpgradeExternalName) {
					...
					// Pre-upgrade was not GUID format (name) - should have been migrated
					klog.V(4).Infof(
